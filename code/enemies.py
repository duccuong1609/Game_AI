...
def aStar(start, end):
    # start = (m.rows,m.cols)
    # gán tất cả các phần tử trong mảng m.grid thành inf (vô cùng)
    g_score={node:float('inf') for node in m.grid}
    g_score[start] = 0
    # f_score là mảng để mà gán khoảng cách giữa các điểm
    f_score={node:float('inf') for node in m.grid}
    f_score[start] = h(start, end)
    open=PriorityQueue()
    # open.put(f(n) = g(n) + h, h, point (first se bang start))
    open.put((h(start, end), h(start, end), start))
    path = {}
    path_dir = {}
    while open:
        # current sẽ là vị trí thứ 2 trong open (tức là = start)
        current = open.get()[2]
        if current == end:
            break
        for d in 'ESNW':
            e, s, w, n = False, False, False, False
            if m.maze_map[current][d] == True:
                if d=='E':
                    childNode=(current[0],current[1]+1)
                    e = True
                if d=='W':
                    childNode=(current[0],current[1]-1)
                    w = True
                if d=='N':
                    childNode=(current[0]-1,current[1])  
                    n = True  
                if d=='S':
                    childNode=(current[0]+1,current[1])
                    s = True
                temp_g_score = g_score[current] + 1
                temp_f_score = temp_g_score + h(childNode,end)
                
                if temp_f_score < f_score[childNode]:
                    g_score[childNode] = temp_g_score
                    f_score[childNode] = temp_f_score
                    open.put((temp_f_score,h(childNode,end),childNode))
                    path[childNode] = current
                    if e:
                        path_dir[childNode] = (0, 1)
                    elif w:
                        path_dir[childNode] = (0, -1)
                    elif n:
                        path_dir[childNode] = (0, -1)
